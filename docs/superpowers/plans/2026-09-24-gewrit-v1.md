...
4. **The Paperless API version is pinned to `9`** (`PAPERLESS_API_VERSION`). A client asking for a version above the server's maximum gets `406`, so the pin is the lowest version that serves every field used here (`id`, `title`, `document_type`, `correspondent`, `created`). Task 5 includes a check against the household's instance (`X-Api-Version` response header) before shipping. `created` is read as its first 10 characters, which works whether the server returns a date or a datetime.
...
In `docs/strategy.md`, delete the two-line backlog item "**Office** module: household document management (until then, documents stay in Library)." (lines 203-204). Find the current phase's list of in-scope items (search for the section that lists the bank-ingestion slice as shipped) and add:
...
- Produces:
  ...
  - `createPaperlessProvider(cfg: Extract<GewritConfig, { provider: 'paperless' }>, o?: { fetchImpl?: typeof fetch; now?: () => number }): DocumentProvider` with `id: 'paperless'`
 ...
 * the server defaults to, which can change the response shape on an upgrade. 9
 * is the lowest version that serves every field read here; a server whose
 * maximum is lower answers 406. Check `X-Api-Version` on the household's
 * instance before raising it.
 */
...
export function createPaperlessProvider(cfg: PaperlessConfig, o: PaperlessOptions = {}): DocumentProvider {
  ...
  return {
    ...
    async openPreview(externalId, opts = {}): Promise<DocumentStream> {
      ...
      // The timeout covers the first byte only: an AbortSignal.timeout would
      // also cut a large PDF off mid-stream.
...
```bash
curl -s -o /dev/null -D - -H "Authorization: Token $PAPERLESS_TOKEN" -H "Accept: application/json; version=9" "$PAPERLESS_BASE_URL/api/documents/?page_size=1" | grep -i -E '^(HTTP|x-api-version)'
```

Expected: `HTTP/... 200` and an `X-Api-Version` of 9 or higher. A `406` means the server's maximum is below 9: stop and report the server's version instead of lowering the constant silently. If no Paperless is reachable from this machine, record "API version check open" in the task report; it is re-checked in Task 13.
...
/**
 * Deletes a link, and the document row with its last link. The document row is
 * locked FOR UPDATE first, so the existence check runs in a NEW statement
 * snapshot after any concurrent createLink on the same document has committed
 * — a NOT EXISTS inside the DELETE could miss that link and cascade it away.
 */
export async function deleteLink(id: string): Promise<boolean> {
  return db.transaction(async (tx) => {
    const [l] = await tx.delete(gewritLinks).where(eq(gewritLinks.id, id)).returning();
    if (!l) return false;
...
Run: `npx vitest run tests/gewrit-service.test.ts`
Expected: PASS (17 tests). If the ordering test fails on `localeCompare` for your locale, it is comparing plain ASCII titles; investigate rather than loosening the assertion.
...
- The Paperless API version is pinned (`PAPERLESS_API_VERSION` in
  `src/modules/gewrit/providers/paperless.ts`). A Paperless older than that
...
- Produces:
  ...
  - `listDocuments(el)`, `searchDocuments(q)`, `createLink(input: CreateLinkInput)`, `updateLink(id, input)`, `deleteLink(id)`, `fetchPreview(documentId)`
...
export function fetchPreview(documentId: string): Promise<Blob> {
  return apiGetBlob(`/gewrit/documents/${documentId}/preview`);
}
```
...
Start the demo stack from the meta repo with the Heorth build from this branch (`deploy/demo-up.sh` builds `../Heorth`) and set `GEWRIT_PROVIDER: fake` locally if Task 13 has not landed yet. Link demo document `1` to any asset, open it, and confirm in **both** Chrome and Firefox that the PDF renders inside the dialog. Record the result in the task report. If a browser refuses to render it, stop and report — do not add a `sandbox` attribute or a CSP as a workaround.
...
- Modify: `heorth-mcp/src/tools/index.ts`, `heorth-mcp/tests/tools.registry.test.ts`, `heorth-mcp/docs/spec/tool-surface.md`
...
Expected: FAIL — cannot resolve `src/tools/gewrit.js`. If the recorded request shape in `tests/helpers/fake-upstream.ts` stores headers under a different key casing, adjust the `authorization` assertion to match `tests/tools.weorc.test.ts`.
...
```bash
cd heorth-mcp
git add src/tools/gewrit.ts src/tools/index.ts tests/tools.gewrit.test.ts tests/tools.registry.test.ts docs/spec/tool-surface.md
...
is pinned to 9; places get their panel from the place manager.
```
