...
> **Note (2026-09-24):** this plans the ADR 0019 proposal, not the built v1 — that is ADR 0017 and [`docs/superpowers/plans/2026-09-24-gewrit-v1.md`](../superpowers/plans/2026-09-24-gewrit-v1.md). Section numbers cited below as "ADR 0019 (draft) §N" are those of the 2026-09-09 draft (see the ADR file's git history); where this plan and ADR 0019's open decisions disagree, the ADR wins.
...
Heorth-owned types, read-only, three methods (ADR 0019 (draft) §3):
...
One table (ADR 0019 (draft) §5–§9). Names follow Heorth's Drizzle conventions, and the
...
(ADR 0019 (draft) §14): finding a document by its *content* is `/search`, which is the
store's job.
...
store (ADR 0019 (draft) §9), and it is worth it: the page survives a paperless outage
and a hundred-document household loads in one query.

**B — Capture** (deferred, ADR 0019): upload from the phone PWA, a pending
...
text in Heorth (ever — ADR 0019 (draft) §14); mirroring paperless's permissions; writing
...
2. ~~Which entity does a *household* document hang on?~~ **Resolved 2026-09-09:
   ...
   (ADR 0019 (draft) §5, §8). A document reaches the register either because a member
...
4. **Backup ownership.** ADR 0019 names this as the real price: paperless holds
...
6. **What does the register do about a document the household untagged but a
   member had anchored?** Decided in ADR 0019 (draft) §9 — it survives, because
